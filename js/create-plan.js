...
Make the plan realistic, engaging, and tailored to the specified difficulty level and learning styles. Include specific topics, activities, and time allocations for each day.`;
  ...
  function renderSchedule(schedule) {
    ...
    // Create a single grid of all days
    const daysGrid = document.createElement('div');
    daysGrid.className = 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4';
    
    allDays.forEach((day) => {
      const dayCard = document.createElement('div');
      dayCard.className = 'bg-white/5 rounded-lg p-4 border border-white/10';
      
      dayCard.innerHTML = `
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            <div class="w-6 h-6 bg-green-500/20 rounded-full flex items-center justify-center">
              <span class="text-green-400 font-bold text-xs">${day.globalDayIndex}</span>
            </div>
            <h4 class="text-green-400 font-medium text-sm">${day.title}</h4>
          </div>
        ...
        <div class="space-y-3">
          <div>
            <h5 class="text-white text-xs font-medium mb-2 flex items-center gap-2">
            ...
            <ul class="space-y-1">
              ${day.topics.slice(0, 3).map(topic => `<li class="text-gray-300 text-xs flex items-start gap-2">
          ...
          <div>
            ...
            <ul class="space-y-1">
              ${day.activities.slice(0, 2).map(activity => `<li class="text-gray-300 text-xs flex items-start gap-2">
  ...
  function showStudyPlan(plan) {
    // Update overview section with actual values from user preferences
    const actualDuration = userPreferences.duration;
    document.getElementById('duration').textContent = `${actualDuration} days`;
  ...
  function handleStartPlan() {
    ...
    if (generatedPlan && currentTopic) {
      const planToSave = {
        topic: currentTopic,
        duration: userPreferences.duration,
        difficulty: userPreferences.difficulty,
        timePerDay: userPreferences.timePerDay,
        overview: generatedPlan.overview,
        schedule: generatedPlan.schedule,
        learningStyles: userPreferences.learningStyles
      };
      
      // Save to localStorage
      const existingPlans = JSON.parse(localStorage.getItem('tutoStudyPlans') || '[]');
      const newPlan = {
        id: Date.now(),
        ...planToSave,
      ...
      localStorage.setItem('tutoStudyPlans', JSON.stringify(existingPlans));
    }
  ...
  function init() {
    ...
    // Set initial time per day selection (30 minutes)
    const initialTimeRadio = document.querySelector('input[name="timePerDay"][value="30"]');
    if (initialTimeRadio) {
      initialTimeRadio.checked = true;
      initialTimeRadio.closest('label').classList.add('border-green-500', 'bg-green-500/10');
    }
